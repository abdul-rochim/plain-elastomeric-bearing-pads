...
W = 37 #in Leveling Pad Width
L = 10 #in Leveling Pad Length
hri = 0.75 #in Leveling Pad Thickness
hrt = hri
...
Ad = 1.25 #ft bearing to FF Abutment
...
file_pep.writelines(["Leveling Pad Width,       W = ", str(W), " in\n"])
file_pep.writelines(["Leveling Pad Length,      L = ", str(L), " in\n"])
file_pep.writelines(["Leveling Pad Thickness, hri = ", str(hri), " in\n\n"])
...
file_pep.writelines(["G = {0:0.2f}".format(G), " ksi\n"])
if G < 0.25 and G > 0.08:
    file_pep.writelines(["Check, 0.08 ksi < G < 0.25 ksi  [ OK ]\n\n"])
else:
    file_pep.writelines(["Check, G < 0.08 ksi and G > 0.25 ksi  [ NOT OK]\n\n"])

file_pep.writelines(["BRIDGE GEOMETRY\n"])
file_pep.writelines(["Profile grade between supports (%) = {0:0.2f}".format(grade_betweensupport), " %\n"])
file_pep.writelines(["bearing to FF Abutment,         Ad = {0:0.2f}".format(Ad), " ft\n\n"])
...
file_pep.writelines(["DL = {0:0.2f}".format(DL), " kip\n\n"]) 
...
file_pep.writelines(["Compressive state\n"])
...
file_pep.writelines(["stress_s_req_1 = 1.00 G Si = {0:0.2f}".format(1.00 * G * Si), " ksi\n"])
file_pep.writelines(["stress_s_req_2 = 0.80 ksi\n"])

file_pep.writelines(["stress_s = DL/ (LW) = {0:0.2f}".format(stress_s), " ksi\n"])
if stress_s <= 1. * G * Si :
...
if stress_s <= 0.80 :
    ...
    file_pep.writelines(["Check, stress_s > 0.80 ksi   [ NOT OK ]\n\n"])

file_pep.writelines(["Compressive Deflection\n"])
...
file_pep.writelines(["dho_d <= 0.09 hri = {0:0.3f}".format(dho_d_req), " in.\n"])
file_pep.writelines(["stress_d = DL/ (LW)\n"])
file_pep.writelines(["stress_d = {0:0.3f}".format(stress_d), " ksi\n"])
...
file_pep.writelines(["dho_d = {0:0.3f}".format(dho_d), " in\n"])
if dho_d < dho_d_req:
    file_pep.writelines(["dho_d = {0:0.3f}".format(dho_d), " in  <  {0:0.3f}".format(dho_d_req), " in  [ OK ]\n\n"]) 
else:
    file_pep.writelines(["dho_d = {0:0.3f}".format(dho_d), " in  >  {0:0.3f}".format(dho_d_req), " in  [ NOT OK ]\n\n"]) 

file_pep.writelines(["Stability\n"])
...
file_pep.writelines(["L/3 = {0:0.2f}".format(L_req), " in\n"])
file_pep.writelines(["W/3 = {0:0.2f}".format(W_req), " in\n"])
file_pep.writelines(["lesser between L/3 and W/3 = {0:0.2f}".format(min_req_L_and_W), " in\n"])

if hrt < min_req_L_and_W :
    file_pep.writelines(["Check: hrt = {0:0.2f}".format(hrt), " in  < {0:0.2f}".format(min_req_L_and_W), " in   [ OK ]\n\n"])
else:
    file_pep.writelines(["Check: hrt = {0:0.2f}".format(hrt), " in  > {0:0.2f".format(min_req_L_and_W), " in   [ NOT OK ]\n\n"])

file_pep.writelines(["Geometry\n"])
